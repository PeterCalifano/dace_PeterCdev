...
namespace DACE
{
    ...
    template <class T>
    void AlgebraicMatrix<T>::resize(int size)
    {
    ...
    template <class T>
    void AlgebraicMatrix<T>::resize(int rows, int cols)
    {
     ...
     ************************************************************************************/
    template <class T>
    T &AlgebraicMatrix<T>::at(const unsigned int irow, const unsigned int icol)
    {
    ...
    template <class T>
    const T &AlgebraicMatrix<T>::at(const unsigned int irow, const unsigned int icol) const
    {
        /*! Reads/write element from/to AlgebraicMatrix.
           \param[in] irow row index.
           \param[in] icol column index.
    ...
    template <class T>
    std::vector<T> AlgebraicMatrix<T>::getrow(const unsigned int irow) const
    {
    ...
    template <class T>
    std::vector<T> AlgebraicMatrix<T>::getcol(const unsigned int icol) const
    {
    ...
    template <class T>
    void AlgebraicMatrix<T>::setrow(const unsigned int irow, const std::vector<T> &obj)
    {
    ...
    template <class T>
    void AlgebraicMatrix<T>::setcol(const unsigned int icol, const std::vector<T> &obj)
    {
    ...
    template <class T>
    AlgebraicMatrix<T> AlgebraicMatrix<T>::submat(const unsigned int first_row, const unsigned int first_col, const unsigned int last_row, const unsigned int last_col) const
    {
    ...
    template <class T>
    AlgebraicMatrix<T> AlgebraicMatrix<T>::submat(const unsigned int last_row, const unsigned int last_col) const
    {
     ...
     ************************************************************************************/
    template <class T>
    AlgebraicMatrix<T> AlgebraicMatrix<T>::transpose() const
    {
    ...
    template <class T>
    unsigned int AlgebraicMatrix<T>::pivot(unsigned int &k, const unsigned int ii, const AlgebraicMatrix<T> &A, std::vector<unsigned int> &P, std::vector<unsigned int> &R, std::vector<unsigned int> &C1, std::vector<unsigned int> &C2, T &det)
    {
        using std::abs;
        ...
        else
        {
            ...
            if (im != k)
                det = -1.0 * det; // adjust sign of determinant
            std::swap(R[k], R[im]);
    ...
    template <class T>
    void AlgebraicMatrix<T>::eliminate(const unsigned int k, AlgebraicMatrix<T> &A, std::vector<unsigned int> &R)
    {
        ...
        for (unsigned int i = 0; i < n; i++)
        {
            if (i != k)
            {
                ...
                {
                    if (j != k)
                        A.at(R[i], j) = A.at(R[i], j) - A.at(R[i], k) * A.at(R[k], j);
                }
    ...
    template <class T>
    AlgebraicMatrix<T> AlgebraicMatrix<T>::inv() const
    {
     ...
     ************************************************************************************/
    template <class T>
    AlgebraicMatrix<double> AlgebraicMatrix<T>::cons() const
    {
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Return the internal data of the AlgebraicMatrix as std::vector
    /**
     * @brief Routines to add interface to Eigen::MatrixXd from AlgebraicMatrix
     * @author PeterC
     * @tparam T
     * @return std::vector<T>
     */
    template <typename T>
    std::vector<T> AlgebraicMatrix<T>::GetInternal_data() const
    {
        return _data;
    };
     ...
     */
    template <typename T>
    Eigen::MatrixXd AlgebraicMatrix<T>::AsEigenMatrix()
    {
        if constexpr (std::is_same<T, DA>::value)
        {
            // Get constant part of the matrix first
            Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> MatrixMap(
                ((cons()).GetInternal_data()).data(), _nrows, _ncols);
